...
const sellStakes = async (req, res) => {
  try {
    ...
    const investments = await Investment.find({
      investorAddress: req.query.investorAddress,
  ...
  try {
    ...
    investments.forEach((investment) => {
      ...
      if (investorAmountMap.hasOwnProperty(investment.investorAddress)) {
        investorAmountMap[investment.investorAddress] +=
          investment.actualAmount;
      } else {
        // If the investor doesn't exist in the map, initialize their total amount
    ...
    const investorTotalAmountArray = Object.keys(investorAmountMap).map(
      (investorAddress) => ({
